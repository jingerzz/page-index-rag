...
def toc_transformer(toc_content, model=None):
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        ...
        if continuation_attempts > 5:
            print("WARNING: toc_transformer exceeded max continuation attempts, using best result")
            break

    # Use extract_json for robustness (handles Extra data, markdown fences, trailing text)
    ...
    if isinstance(parsed, dict) and 'table_of_contents' in parsed:
        ...
        # LLM returned the array directly without the wrapper object
        cleaned_response = convert_page_to_int(parsed)
    else:
        raise Exception(f"toc_transformer: unexpected JSON structure: {type(parsed)}")
    return cleaned_response
...
def add_page_number_to_toc(part, structure, model=None):
    ...
    for item in json_result:
        if isinstance(item, dict) and 'start' in item:
            del item['start']
    return [item for item in json_result if isinstance(item, dict)]
...
def process_none_page_numbers(toc_items, page_list, start_index=1, model=None):
    for i, item in enumerate(toc_items):
        if "physical_index" not in item:
            ...
            if result and isinstance(result[0], dict) and result[0].get('physical_index'):
                pi = result[0]['physical_index']
